...
static constexpr uint16_t AX25_CRC_CORRECT = 0xF0B8;
static constexpr uint16_t CRC_CCITT_INIT_VAL = 0xFFFF;
...
private:
    ...
    void hdlcProcessBit(int bit) {
        ...
        if (h.in_frame && h.bit_pos == 7 && h.frame_size >= afsk::detail::AFSK_MIN_FRAME_SIZE) {
                if (afsk::detail::afsk_crc_calc(h.frame, (size_t)h.frame_size) == afsk::detail::AX25_CRC_CORRECT) {
